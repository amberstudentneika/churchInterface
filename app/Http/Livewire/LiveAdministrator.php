...
class LiveAdministrator extends Component
{
    ...
    public function removeUser($id){
        $ch=curl_init();
        $url = 'http://192.168.0.3:8081/api/deactivate/user/'.$id;
...
public function makeAdmin($id){
    $ch=curl_init();
    $url = 'http://192.168.0.3:8081/api/member/role/update/admin/'.$id;
    $memberToken=session()->get('memberToken');
    $headers=[
        'Accept: application/json',
        'Authorization: Bearer '.$memberToken
    ...
    $ch=curl_init();
    $url = 'http://192.168.0.3:8081/api/admin/role/update/member/'.$id;
    $memberToken=session()->get('memberToken');
    $headers=[
        'Accept: application/json',
        'Authorization: Bearer '.$memberToken
    ...
    $ch=curl_init();
    $url = 'http://192.168.0.3:8081/api/reactivate/user/'.$id;
    $memberToken=session()->get('memberToken');
    $headers=[
        'Accept: application/json',
        'Authorization: Bearer '.$memberToken
    ...
    public function render()
    {
         //view Users
         $ch=curl_init();
         $url = 'http://192.168.0.3:8081/api/member/index';
         $memberToken=session()->get('memberToken');
        $headers=[
            'Accept: application/json',
            'Authorization: Bearer '.$memberToken
